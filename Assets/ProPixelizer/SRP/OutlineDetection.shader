	...
	Properties{
		...
		Pass{
			...
			inline float checkMatchingIDsForCrease(float2 mainTexel, float2 gradientDir, float ID, float pixelSize) {
				...
				float4 neighbourA = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainTexel + gradientDir * _TexelSize.xy * pixelSize);
				float4 neighbourB = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainTexel - gradientDir * _TexelSize.xy * pixelSize);
			...
			inline float countNeighbourID(float2 mainTexel, float2 neighbour, float pixelSize, float ID, float depth) {
				...
				float4 neighbourD = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, npos);

				#if DEPTH_TEST_OUTLINES_ON
				float neighbourDepth = SAMPLE_TEXTURE2D_X(_MainTex_Depth, sampler_MainTex_Depth, npos).r;
					#if UNITY_REVERSED_Z
			...
			inline float3 getOutlineNormals(float2 uv) {
				float2 rg = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv).rb * 2 - 1;
			...
			void frag(v2f i, out float4 color: COLOR) {
				...
				float4 packedData = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainTexel);
				...
				#if DEPTH_TEST_OUTLINES_ON
					float depth = SAMPLE_TEXTURE2D_X(_MainTex_Depth, sampler_MainTex_Depth, mainTexel).r;
				#else
					float depth = 0;
				#endif
